...
class aman extends Controller
{
    public function index()
    {
        ...
        $duaTahunSetelahTerbaru = Carbon::parse($penjualanTerbaru->waktu_penjualan)->addYears(1);
        $tigaTahunSetelahTerbaru = Carbon::parse($penjualanTerbaru->waktu_penjualan)->addYears(3);
        // dd($duaTahunSetelahTerbaru, $duaTahunSetelahTerlama);

        $periodePenjualanAwal = Penjualan::where('waktu_penjualan', '<', $duaTahunSetelahTerbaru)
    ...
    public function forecast(Request $request)
    {
        ...
        // dd($periodeAwal, $periodeAkhir, $batasDuaTahun);
        ...
        for ($alpha = 0.1; $alpha <= 1.0; $alpha += 0.1) {
            for ($beta = 0.1; $beta <= 1.0; $beta += 0.1) {
                for ($gamma = 0.1; $gamma <= 1.0; $gamma += 0.1) {
                    // Hitung peramalan dan MAPE dengan kombinasi parameter ini
                    $forecast = calculateHoltWintersForecast($dataPenjualan, $alpha, $beta, $gamma);
                    $mape = calculateMAPE($dataPenjualan, $forecast);
